#!/usr/bin/env python3
"""
Crawl the Relic/WorldsEdge Community API for recent AoE2 ranked matches.
Outputs public/data/maps.json (2v2) or public/data/maps_3v3.json (3v3).

Run locally:  python3 scripts/crawl.py
              python3 scripts/crawl.py --mode 3v3
Batch mode:   python3 scripts/crawl.py --start-rank 500
CI:           python3 scripts/crawl.py --count 15000
...
MODES = {
    "2v2": {
        ...
        "state": Path(".data_cache/crawl_state.pkl"),
    },
    "3v3": {
        "match_type": 8,
        "team_size": 3,
        "total_members": 6,
        "output": Path("public/data/maps_3v3.json"),
        "state": Path(".data_cache/crawl_state_3v3.pkl"),
    },
}
...
def load_state(state_path: Path) -> tuple[set, dict]:
    if state_path.exists():
        with open(state_path, "rb") as f:
            state = pickle.load(f)
        seen = state["seen"]
        stats = defaultdict(lambda: {"games": 0, "wins": 0}, state["stats"])
        print(f"  Loaded state: {len(seen):,} seen matches, {len(stats):,} civ-combo keys")
    else:
        seen = set()
...
def save_state(seen: set, stats: dict, state_path: Path) -> None:
...
def write_output(stats: dict, seen: set, output_path: Path, team_size: int, label: str = "") -> None:
    map_rows: dict[str, list] = defaultdict(list)
    for key, s in stats.items():
        map_key = key[0]
        civs = list(key[1:])
        map_rows[map_key].append((civs, s["games"], s["wins"]))
    ...
    for map_key, rows in sorted(map_rows.items()):
        ...
        if total < MIN_MAP_APPEARANCES:
            continue
        top = sorted(rows, key=lambda r: r[1], reverse=True)[:TOP_N]
        output["maps"][map_key] = {
            ...
            "teams": [
                {
                    ...
                    "games": r[1],
                    "wins": r[2],
                    "playrate": round(r[1] / total, 6),
                    "winrate": round(r[2] / r[1], 4),
                }
                for r in top
            ],
        }
...
def main() -> None:
    ...
    parser.add_argument("--mode", choices=["2v2", "3v3"], default="2v2",
                        help="Match mode to crawl")
    ...
    match_type = mode["match_type"]
    team_size = mode["team_size"]
    total_members = mode["total_members"]
    output_path = mode["output"]
    state_path = mode["state"]
    ...
    seen, stats = load_state(state_path)

    # 4. Crawl this batch
    print(f"\nCrawling {args.mode} match histories ({DAYS_BACK}-day window) …")
    for i, pid in enumerate(batch_ids):
        if i % 100 == 0:
            print(f"  [{i}/{len(batch_ids)}] {len(seen):,} unique {args.mode} matches so far")
        try:
        ...
        for match in matches:
            if match.get("matchtype_id") != match_type:
                continue
            ...
            if len(team_lists[0]) != team_size or len(team_lists[1]) != team_size:
                continue
        ...
        if (i + 1) % CHECKPOINT_EVERY == 0:
            save_state(seen, stats, state_path)
            write_output(stats, seen, output_path, team_size, label=f"rank {args.start_rank + i + 1}/{total_players}")

    print(f"\n  Batch done. Total unique {args.mode} matches so far: {len(seen):,}")

    # 5. Save state + write final output
    save_state(seen, stats, state_path)
    write_output(stats, seen, output_path, team_size, label=f"ranks {args.start_rank + 1}–{end_rank} of {total_players}")
    print("Done.")
